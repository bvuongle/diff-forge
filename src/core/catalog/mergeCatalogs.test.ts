...
describe('mergeCatalogs', () => {
  ...
  it('concatenates components from distinct repos', () => {
    const a = make([{ type: 'A', source: 's1', version: '1.0', implements: [], requires: [], config: {} }])
    const b = make([{ type: 'B', source: 's2', version: '1.0', implements: [], requires: [], config: {} }])
  ...
  it('deduplicates on (source, type, version); first wins', () => {
    const a = make([{ type: 'A', source: 's', version: '1.0', implements: ['v1'], requires: [], config: {} }])
    const b = make([{ type: 'A', source: 's', version: '1.0', implements: ['v2'], requires: [], config: {} }])
    const merged = mergeCatalogs([a, b])
  ...
  it('keeps different versions of the same component', () => {
    const a = make([
      { type: 'A', source: 's', version: '1.0', implements: [], requires: [], config: {} },
      { type: 'A', source: 's', version: '2.0', implements: [], requires: [], config: {} }
    ])
