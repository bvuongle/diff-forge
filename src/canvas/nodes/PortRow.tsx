...
function PortRow({ slot, nodeId, side, isConnected, dragInfo, tooltipText, hideLabel, onMouseDown }: PortRowProps) {
  ...
  const row = (
    ...
    >
      <Box
        ...
        onMouseDown={(e) => {
          if (ref.current && !isDragDimmed) onMouseDown(e, nodeId, slot.name, ref.current)
        }}
        sx={portSx}
      />
      {!hideLabel && side === 'left' && (
        <>
          <Typography
            ...
            color="text.primary"
            noWrap
            sx={{ fontSize: '0.7rem', opacity: isDragDimmed ? 0.3 : 1 }}
          >
            {slot.name}
          </Typography>
          <Chip
  ...
  if (tooltipText) {
    return (
      <Tooltip title={tooltipText} placement={side === 'left' ? 'left' : 'right'} arrow>
        {row}
      </Tooltip>
    )
  }
